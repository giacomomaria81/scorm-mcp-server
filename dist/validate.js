...
const WRAPPER_12 = '<?xml version="1.0" encoding="UTF-8"?>\n' +
...
export async function validatePackage(opts) {
    const checks = [];
    const errors = [];
    const warnings = [];
    ...
    try {
        ...
        push("zip-readable", "Input file could not be read", false, String(e.message ?? e));
        return report({});
    }
    let zip;
    try {
        ...
        push("zip-readable", "Input is not a valid zip archive", false, "The file could not be opened as a zip. SCORM packages must be plain .zip files (no rar/7z, no password).");
        return report({});
    }
    ...
    if (!rootManifestName) {
        if (nested) {
            push("manifest-at-root", "imsmanifest.xml is not at the ROOT of the zip", false, `Found "${nested}" instead. The course folder itself was zipped. Re-zip the CONTENTS of the folder ` +
                "(imsmanifest.xml must be the first thing an LMS sees when it opens the archive).");
        }
        else {
            push("manifest-at-root", "imsmanifest.xml is missing", false, "No manifest anywhere in the archive: this is not a SCORM package (or the export is incomplete).");
        }
    ...
    try {
        ...
        if ($("manifest").length === 0) {
            throw new Error("no <manifest> root element");
        }
        push("manifest-parses", "imsmanifest.xml is well-formed XML with a <manifest> root", true);
    }
    catch (e) {
        push("manifest-parses", "imsmanifest.xml could not be parsed", false, String(e.message ?? e));
        return report({ filesInZip });
    }
    // -- 4. SCORM edition -------------------------------------------------------
    ...
    if (/2004|CAM 1\.3/i.test(schemaversion) || (ns2004 && !ns12)) {
        scormVersion = "2004";
    }
    else if (/^1\.2$/.test(schemaversion) || (ns12 && !ns2004)) {
        scormVersion = "1.2";
    }
    push("version-detected", "SCORM edition identified", scormVersion !== "unknown", scormVersion !== "unknown"
    ...
    push("organization", "An <organization> with a launchable <item identifierref=…> exists", org.length > 0 && item.length > 0, org.length === 0 ? "No <organization> found." : item.length === 0 ? "No <item> carries an identifierref." : undefined);
    // -- 6. launch resource -----------------------------------------------------
    let entryHref;
    if (item.length > 0) {
        ...
        push("launch-resource", "The item's identifierref resolves to a launchable <resource href=…>", ok, resource.length === 0 ? `No <resource identifier="${ref}"> found.` :
    ...
    if (entryHref) {
        ...
        push("entry-exists", "The launch file exists in the zip", zipSet.has(decodedEntry) || zipLower.has(decodedEntry.toLowerCase()), zipSet.has(decodedEntry) ? undefined : `"${decodedEntry}" not found in the archive.`);
    }
    const shown = missing.slice(0, 8).join(", ") + (missing.length > 8 ? ` … (+${missing.length - 8} more)` : "");
    push("files-exist", "Every <file href> listed in the manifest exists in the zip", missing.length === 0, missing.length ? "Missing: " + shown : undefined);
        ...
        try {
            ...
            try {
                for (const n of await fs.readdir(fallbackDir)) {
                    if (!n.toLowerCase().endsWith(".xsd")) {
                        continue;
                    }
                    try {
                        await fs.access(path.join(tmp, n));
                    }
                    catch {
                        await fs.copyFile(path.join(fallbackDir, n), path.join(tmp, n));
                    }
                }
            }
            ...
            if (scormVersion === "1.2") {
                await fs.writeFile(path.join(tmp, "wrapper12.xsd"), WRAPPER_12);
            }
            try {
...
export function formatReport(r) {
    ...
    if (r.schemaValidation === "skipped") {
        lines.push("  (XSD validation skipped)");
    }
    return lines.join("\n");
}
