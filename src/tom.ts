...
// --------------------------------------------------------------------------
// Minimal .xlsx reader (an xlsx is a zip of XML — JSZip is already a dep)
// --------------------------------------------------------------------------

type Sheet = Map<string, string>; // "A1" -> text value
...
function textOf(xmlFragment: string): string {
  ...
  while ((m = re.exec(xmlFragment))) { out += xmlDecode(m[1]); }
  return out;
}
...
function parseSheetXml(xml: string, shared: string[]): Sheet {
  ...
  while ((m = re.exec(xml))) {
    ...
    if (trimmed !== "") { cells.set(ref.toUpperCase(), trimmed); }
  }
  return cells;
}

export interface Workbook { sheets: { name: string; cells: Sheet }[] }

/** Read an .xlsx buffer into named sheets of cell maps. */
export async function readXlsx(buf: Buffer): Promise<Workbook> {
  const zip = await JSZip.loadAsync(buf);
  const get = async (p: string): Promise<string | null> => {
    const f = zip.file(p) ?? zip.file(p.replace(/^\//, ""));
    return f ? f.async("string") : null;
  };
  const wbXml = await get("xl/workbook.xml");
  if (!wbXml) { throw new Error("xlsx: xl/workbook.xml missing"); }
  ...
  const sheets: { name: string; cells: Sheet }[] = [];
  {
    ...
    while ((m = re.exec(wbXml))) {
      ...
      if (!target) { target = `xl/worksheets/sheet${fallbackIndex}.xml`; }
      const xml = await get(target);
      if (xml !== null) { sheets.push({ name, cells: parseSheetXml(xml, shared) }); }
      fallbackIndex++;
    }
  }
  return { sheets };
...
export interface TomQuizQuestion {
  question: string; media?: string;
...
export type TomCard =
  ...
  | { kind: "flash"; frontTitle?: string; front: string; button?: string; backTitle?: string; back: string }
  | { kind: "quiz"; q: TomQuizQuestion };

export interface TomActivity {
  title: string;
  type: "mobile-course" | "quiz-game" | "content";
  introTitle?: string; introText?: string;
  cards: TomCard[];
}
export interface TomCourse {
  title: string;
  language?: string;
...
function findSheet(wb: Workbook, patterns: RegExp[], fallbackIndex: number | null): Sheet | null {
  for (const p of patterns) {
    const s = wb.sheets.find((sh) => p.test(norm(sh.name)));
    if (s) { return s.cells; }
  }
  if (fallbackIndex !== null && wb.sheets[fallbackIndex]) { return wb.sheets[fallbackIndex].cells; }
  return null;
}

/** Parse the file name "Course - Activity - EN.xlsx" (tolerant). */
export function parseTomName(base: string): { course?: string; activity?: string; language?: string } {
  const stem = base.replace(/\.xlsx$/i, "");
  const parts = stem.split(/\s+-\s+/);
  if (parts.length === 1) { return { activity: stem }; }
  let language: string | undefined;
  if (/^[a-z]{2}(?:-[a-z]{2})?$/i.test(parts[parts.length - 1])) { language = parts.pop()!.toLowerCase(); }
...
export function parseTomActivity(wb: Workbook, name: string, warnings: string[]): TomActivity {
  ...
  const act: TomActivity = { title, type: "content", cards: [] };
  if (config) {
  ...
  for (const r of sorted) {
    ...
    if (!a || HEADER_WORDS.has(a)) { continue; }
    considered++;
    if (CARD_TYPES.has(a)) { mcVotes++; }
    else if (row.get(COL("E")) && (row.get(COL("H")) || row.get(COL("K")))) { qgVotes++; }
  }
  ...
  if (considered > 0 && mcVotes >= Math.max(1, considered / 2)) {
    ...
    for (const r of sorted) {
      ...
      if (!CARD_TYPES.has(kind)) { continue; }
      if (kind === "info") {
        const text = row.get(COL("E")) ?? "";
        if (text || row.get(COL("B"))) { act.cards.push({ kind: "info", title: row.get(COL("B")), text }); }
      } else if (kind === "transition") {
        const text = row.get(COL("E")) ?? "";
        if (text) { act.cards.push({ kind: "transition", text }); }
      } else if (kind === "flash") {
        act.cards.push({
          kind: "flash",
        ...
        const q: TomQuizQuestion = {
          question: row.get(COL("Q")) ?? "", media: row.get(COL("T")),
          answers: [], correction: row.get(COL("AD")),
        };
        const ans = (colLetters: string, correct: boolean): void => {
          const t = row.get(COL(colLetters));
          if (t) { q.answers.push({ text: t, correct }); }
        };
        ans("U", true); ans("X", false); ans("AA", false);
        if (q.question && q.answers.length >= 2) { act.cards.push({ kind: "quiz", q }); }
      }
    }
  } else if (qgVotes > 0) {
    act.type = "quiz-game";
    for (const r of sorted) {
      ...
      if (!question || HEADER_WORDS.has(norm(question))) { continue; }
      const q: TomQuizQuestion = {
      ...
      const ans = (colLetters: string, correct: boolean): void => {
        const t = row.get(COL(colLetters));
        if (t && !/^(answer|reponse)\b/.test(norm(t))) { q.answers.push({ text: t, correct }); }
      };
      ans("E", true); ans("H", false); ans("K", false); ans("N", false);
      if (q.answers.length >= 2) { act.cards.push({ kind: "quiz", q }); }
    }
  } else {
    ...
    for (const r of sorted) {
      ...
      if (text && !HEADER_WORDS.has(norm(parts[0]))) { act.cards.push({ kind: "info", text }); }
    }
  }
...
function mediaHref(name: string, mediaIndex: Map<string, string>): string | null {
  const key = name.trim().toLowerCase().split("/").pop() ?? "";
  return mediaIndex.get(key) ?? null;
}

function renderMedia(name: string, opts: string | undefined, label: string | undefined,
  mediaIndex: Map<string, string>, warnings: string[]): string {
  ...
  if (ext === "mp4") { return `<video class="media-video" src="${h}" controls playsinline></video>`; }
  if (ext === "mp3") { return `<audio class="media-audio" src="${h}" controls></audio>`; }
  if (ext === "pdf") { return `<p><a class="media-pdf" href="${h}" target="_blank" rel="noopener">${escapeHtml(label || "Open document")}</a></p>`; }
  return `<p><a href="${h}" target="_blank" rel="noopener">${escapeHtml(label || name)}</a></p>`;
}

/** Convert one text cell (with TOM codes) to safe HTML. */
export function tomTextToHtml(text: string, mediaIndex: Map<string, string>, warnings: string[]): string {
  ...
  for (const rawLine of lines) {
    ...
    if (media) { out.push(renderMedia(media[1], media[2], media[3], mediaIndex, warnings)); continue; }
    ...
    if (heading) {
      ...
      let cls = "";
      if (inner.startsWith(">><<")) { cls = " class=\"center\""; inner = inner.slice(4).trim(); }
    ...
    let cls = "";
    if (line.startsWith(">><<")) { cls = "center"; line = line.slice(4).trim(); }
    else if (line.startsWith(">>>>")) { cls = "right"; line = line.slice(4).trim(); }
    else if (line.startsWith("<<<<")) { cls = "left"; line = line.slice(4).trim(); }
    else if (line.startsWith("!!")) { cls = "highlight"; line = line.slice(2).trim(); }

    // inline [media:...] inside a sentence
    let html = escapeHtml(line);
    html = html.replace(/\[media:([^,\]|]+)(?:,([^\]|]+))?(?:\|([^\]]+))?\]/gi,
      (_m, n: string, o: string | undefined, l: string | undefined) =>
        renderMedia(xmlDecode(n), o, l ? xmlDecode(l) : undefined, mediaIndex, warnings));
...
export function renderTomCourseHtml(course: TomCourse, mediaIndex: Map<string, string>): string {
  ...
  course.activities.forEach((act, ai) => {
    ...
    for (const card of act.cards) {
      if (card.kind === "info") {
        ...
        parts.push(
          `<article class="card flash" data-flash>` +
        ...
        const answers = shuffleDeterministic(q.answers, quizCount)
          ...
          .join("");
        parts.push(
          `<article class="card quiz" data-quiz>` +
...
function actLabel(t: TomActivity["type"]): string {
  if (t === "quiz-game") { return "Quiz"; }
  if (t === "mobile-course") { return "Course"; }
  return "Content";
}
...
export async function parseTomExport(
  ...
  for (const rel of xlsx) {
    ...
    if (!course && meta.course) { course = meta.course; }
    ...
    try {
      ...
      warnings.push(`TOM: cannot read "${base}" (${(e as Error).message}) - skipped.`);
    }
  }
...
export function buildMediaIndex(files: string[]): Map<string, string> {
  const idx = new Map<string, string>();
  for (const f of files) {
    if (isXlsx(f)) { continue; }
