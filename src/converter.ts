...
export interface BuildOptions {
  ...
  identifier?: string;
  language?: string;
  /** Auto-generate 'view' milestones from document structure when none declared. Default: true. */
  autoMilestones?: boolean;
  /** Report cmi.success_status='passed' on completion (adds data-scorm-success). Default: false. */
  successOnCompletion?: boolean;
  /** Mastery threshold 0..1: enables score-based success + manifest objectives. */
  masteryScore?: number;
  /** Vendor CDN libraries (React/Babel…) into the package via window.__resources. Default: true. */
  vendorCdn?: boolean;
...
export interface BuildResult {
  ...
  warnings: string[];
  inlinedBytes: number;
  schemasBundled: number;
  milestonesAuto: boolean;
...
type ResolvedRef =
  ...
  | { kind: "file"; file: string; key: string; root: string }
  | { kind: "skip" };

interface Ctx {
  warnings: string[];
  bytes: number;
  cache: Map<string, string | null>; // resolved key -> data URI (or null on failure)
  baseUrl?: string;
  baseDir?: string;
}
 ...
 */
function encodeHref(f: string): string {
...
function resolveRef(ref: string, baseUrl?: string, baseDir?: string, warnings?: string[]): ResolvedRef {
  const r = ref.trim();
  if (!r || r.startsWith("data:") || r.startsWith("#") || r.startsWith("blob:") || r.startsWith("mailto:") || r.startsWith("javascript:") || r.startsWith("tel:")) {
    return { kind: "skip" };
  }
  if (/^https?:\/\//i.test(r)) { return { kind: "url", url: r, key: "u:" + r }; }
  if (r.startsWith("//")) { const u = "https:" + r; return { kind: "url", url: u, key: "u:" + u }; }
  if (baseUrl) {
    try { const u = new URL(r, baseUrl).toString(); return { kind: "url", url: u, key: "u:" + u }; } catch { return { kind: "skip" }; }
  }
  if (baseDir) {
    ...
    if (rel.startsWith("..") || path.isAbsolute(rel)) {
      ...
      if (warnings) { warnings.push("Référence hors du dossier du module (non inlinée): " + r); }
      return { kind: "skip" };
    }
...
function refLabel(resolved: ResolvedRef): string {
...
async function mapLimit<T, R>(items: T[], limit: number, fn: (item: T) => Promise<R>): Promise<R[]> {
  ...
  for (let w = 0; w < n; w++) {
    workers.push((async () => { while (i < items.length) { const idx = i++; out[idx] = await fn(items[idx]); } })());
  }
...
async function fetchBuffer(resolved: ResolvedRef): Promise<{ data: Buffer; contentType: string } | null> {
...
async function dataUriFor(resolved: ResolvedRef, ctx: Ctx): Promise<string | null> {
  if (resolved.kind === "skip") { return null; }
  if (ctx.cache.has(resolved.key)) { return ctx.cache.get(resolved.key) ?? null; }
  const asset = await fetchBuffer(resolved);
  if (!asset) { ctx.cache.set(resolved.key, null); return null; }
...
async function fetchText(resolved: ResolvedRef): Promise<string | null> {
  const asset = await fetchBuffer(resolved);
...
async function inlineCssUrls(css: string, baseUrl: string | undefined, baseDir: string | undefined, ctx: Ctx): Promise<string> {
  const refs = new Set<string>();
  let m: RegExpExecArray | null;
  URL_RE.lastIndex = 0;
  while ((m = URL_RE.exec(css)) !== null) { if (m[2] && !m[2].startsWith("data:")) { refs.add(m[2]); } }
  ...
  const pairs = await mapLimit(list, FETCH_CONCURRENCY, async (ref) => {
    ...
    if (resolved.kind === "skip") { return [ref, null] as const; }
    const uri = await dataUriFor(resolved, ctx);
    if (!uri) { ctx.warnings.push("CSS url() non inliné: " + refLabel(resolved)); }
    return [ref, uri] as const;
  });
...
async function processCss(css: string, baseUrl: string | undefined, baseDir: string | undefined, ctx: Ctx, depth: number, visited: Set<string>): Promise<string> {
  ...
  if (depth < MAX_IMPORT_DEPTH) {
    ...
    while ((m = IMPORT_RE.exec(css)) !== null) {
      const ref = m[2] || m[4];
      if (ref && !ref.startsWith("data:")) { imports.push({ full: m[0], ref }); }
    }
    for (const imp of imports) {
      const resolved = resolveRef(imp.ref, baseUrl, baseDir, ctx.warnings);
      if (resolved.kind === "skip" || visited.has(resolved.key)) { css = css.replace(imp.full, ""); continue; }
      visited.add(resolved.key);
      const importedRaw = await fetchText(resolved);
      if (importedRaw == null) { ctx.warnings.push("@import non inliné: " + refLabel(resolved)); css = css.replace(imp.full, ""); continue; }
      let childBaseUrl = baseUrl, childBaseDir = baseDir;
      if (resolved.kind === "url") { childBaseUrl = resolved.url; childBaseDir = undefined; }
      else if (resolved.kind === "file") { childBaseDir = path.dirname(resolved.file); childBaseUrl = undefined; }
...
async function inlineSrcset(srcset: string, ctx: Ctx): Promise<string> {
  ...
  const rebuilt = await mapLimit(parts, FETCH_CONCURRENCY, async (part) => {
    ...
    if (resolved.kind === "skip") { return part; }
    const uri = await dataUriFor(resolved, ctx);
    if (!uri) { ctx.warnings.push("srcset non inliné: " + refLabel(resolved)); return part; }
...
// minimal typing for cheerio nodes we pass around
type El = Parameters<cheerio.CheerioAPI>[0];

async function inlineAssets(html: string, ctx: Ctx): Promise<string> {
  const $ = cheerio.load(html);

  // ensure <head>, charset and a mobile-first viewport
  if ($("head").length === 0) {
    if ($("html").length) { $("html").prepend("<head></head>"); } else { $.root().prepend("<head></head>"); }
  }
  ...
  await mapLimit(linkEls, FETCH_CONCURRENCY, async (el) => {
    ...
    if (cssRaw == null) { ctx.warnings.push("Feuille de style non inlinée: " + href); return; }
    let cssBaseUrl = ctx.baseUrl, cssBaseDir = ctx.baseDir;
    if (resolved.kind === "url") { cssBaseUrl = resolved.url; cssBaseDir = undefined; }
    else if (resolved.kind === "file") { cssBaseDir = path.dirname(resolved.file); cssBaseUrl = undefined; }
  ...
  await mapLimit(scriptEls, FETCH_CONCURRENCY, async (el) => {
    ...
    if (jsText == null) { ctx.warnings.push("Script non inliné: " + src); return; }
    $(el).removeAttr("src");
  ...
  for (const sel of ["img[src]", "source[src]", "video[src]", "audio[src]", "[poster]"]) {
    ...
    await mapLimit(nodes, FETCH_CONCURRENCY, async (el) => {
      ...
      const uri = await dataUriFor(resolved, ctx);
      if (!uri) { ctx.warnings.push("Média non inliné (" + attr + "): " + refLabel(resolved)); return; }
      $(el).attr(attr, uri);
    });
  }
 ...
 */
function autoTagMilestones(html: string): { html: string; applied: boolean } {
  const $ = cheerio.load(html);
  if ($("[data-jalon]").length > 0) { return { html, applied: false }; }
  let candidates = $("section").toArray();
  if (candidates.length === 0) { candidates = $("article").toArray(); }
  if (candidates.length === 0) { candidates = $("h2").toArray(); }
  if (candidates.length === 0) { candidates = $("h3").toArray(); }
  candidates = candidates.filter((el) => $(el).text().trim().length > 0);
  if (candidates.length === 0) { return { html, applied: false }; }
  let chosen = candidates;
  if (candidates.length > AUTO_MILESTONES_MAX) {
    const n = candidates.length;
    const picked = new Set<number>();
    for (let i = 0; i < AUTO_MILESTONES_MAX; i++) { picked.add(Math.round((i * (n - 1)) / (AUTO_MILESTONES_MAX - 1))); }
...
function injectRuntime(html: string, language?: string, successOnCompletion?: boolean, masteryScore?: number): { html: string; milestoneIds: string[] } {
  const $ = cheerio.load(html);
  ...
  if (language && $("html").length && !$("html").attr("lang")) { $("html").attr("lang", language); }
...
function buildManifest(identifier: string, title: string, _language: string, entryHref = "index.html", extraFiles: string[] = [], masteryScore?: number): string {
 ...
 */
const TRACKING_SIGNAL_RE =
  /data-jalon|data-scorm-success|SCORM2004\s*\.\s*(reach|declare|score)|["'](?:scorm|dc):(?:progress|complete|score)["']/;

export function hasTrackingSignal(bundle: Bundle, entryHtml: string, supportJs: string): boolean {
  if (TRACKING_SIGNAL_RE.test(entryHtml) || TRACKING_SIGNAL_RE.test(supportJs)) { return true; }
  for (const rel of bundle.files) {
    if (!/\.(html?|js|mjs|jsx|ts|tsx)$/i.test(rel)) { continue; }
    try {
      if (TRACKING_SIGNAL_RE.test(readFileSync(path.join(bundle.root, rel), "utf8"))) { return true; }
...
// --------------------------------------------------------------------------
// v2: bundle input (.zip / directory) + Claude Design pipeline
// --------------------------------------------------------------------------

interface Bundle { root: string; files: string[]; entryRel: string; cleanup: boolean; warnings?: string[]; suggestedTitle?: string }

const SKIP_FILES = new Set([".ds_store", "thumbs.db", ".thumbnail"]);
function keepBundleFile(rel: string): boolean {
  const parts = rel.split("/");
  for (const p of parts) {
    const low = p.toLowerCase();
    if (SKIP_FILES.has(low) || low === ".git" || low === "node_modules" || p.startsWith("._")) { return false; }
  }
  return true;
}
...
async function walkDir(root: string, base = ""): Promise<string[]> {
  ...
  for (const e of entries) {
    ...
    if (!keepBundleFile(rel)) { continue; }
    if (e.isDirectory()) { out.push(...(await walkDir(root, rel))); }
    else if (e.isFile()) { out.push(rel); }
  }
  return out;
}

function pickEntry(files: string[]): string {
  const dc = files.filter((f) => f.toLowerCase().endsWith(".dc.html"));
  if (dc.length) { return dc.sort((x, y) => x.split("/").length - y.split("/").length)[0]; }
...
async function loadBundle(inputPath: string): Promise<Bundle> {
  ...
  for (const [name, entry] of Object.entries(zin.files)) {
    ...
    if (!keepBundleFile(clean)) { continue; }
    const dest = path.join(root, clean);
    if (!path.resolve(dest).startsWith(root + path.sep) && path.resolve(dest) !== root) { continue; } // zip-slip
    await fs.mkdir(path.dirname(dest), { recursive: true });
    await fs.writeFile(dest, await (entry as JSZip.JSZipObject).async("nodebuffer"));
  }
  try {
    const files = await walkDir(root);
    if (isTomExport(files)) { return await tomBundle(root, files, true, fallbackTitle); }
...
function detectFormat(html: string): "claude-design" | "html" {
...
async function vendorCdnLibs(supportJs: string, ctx: Ctx): Promise<{ mapping: Record<string, string>; vendorFiles: Map<string, Buffer>; vendored: string[] }> {
  ...
  for (const url of urls) {
    ...
    if (!asset) { ctx.warnings.push("Lib CDN non vendorisée (téléchargement impossible): " + url); continue; }
...
function buildInjection(mapping: Record<string, string> | null, masteryScore: number | undefined): string {
  let block = "";
...
function injectIntoDcHtml(html: string, injection: string): string {
  const m = html.match(/<script[^>]*src\s*=\s*["'][^"']*support\.js(?:\?[^"']*)?["'][^>]*>/i);
  if (m && m.index != null) { return html.slice(0, m.index) + injection + html.slice(m.index); }
  const head = html.search(/<\/head>/i);
  if (head >= 0) { return html.slice(0, head) + injection + html.slice(head); }
...
export async function buildPackage(opts: BuildOptions): Promise<BuildResult> {
  ...
  if (opts.inputPath) {
    ...
    if (st && (st.isDirectory() || /\.zip$/i.test(opts.inputPath))) { bundle = await loadBundle(opts.inputPath); }
  }
  if (!title && bundle?.suggestedTitle) { title = bundle.suggestedTitle; }
  if (!title) {
  ...
  if (bundle) {
    if (bundle.warnings) { ctx.warnings.push(...bundle.warnings); }
    try {
      ...
      if (fmt === "claude-design") {
        ...
        if (opts.vendorCdn === false) {
          if (cdnUrls.length) { ctx.warnings.push("Vendorisation désactivée : " + cdnUrls.length + " lib(s) CDN non embarquée(s) — en LMS à réseau filtré, le module pourrait ne pas démarrer."); }
        } else if (cdnUrls.length) {
          const v = await vendorCdnLibs(supJs, ctx);
          mapping = Object.keys(v.mapping).length ? v.mapping : null;
          for (const [k, b] of v.vendorFiles) { vendorFiles.set(k, b); }
          vendored.push(...v.vendored);
          if (!mapping) { ctx.warnings.push("Libs CDN détectées mais non vendorisées (réseau de build ?) : en LMS à réseau filtré, le module pourrait ne pas démarrer."); }
        }
        const injection = buildInjection(mapping, mastery);
        let outEntry = injectIntoDcHtml(entryHtml, injection);
        if (!/^\s*<!doctype/i.test(outEntry)) { outEntry = "<!DOCTYPE html>\n" + outEntry; }
        ...
        if (!hasTrackingSignal(bundle, entryHtml, supJs)) {
          ctx.warnings.push(
            "AUCUN SUIVI : ce bundle ne déclare aucun signal de progression. Le module s'importera et " +
      ...
      let working = inlined; let milestonesAuto = false;
      if (opts.autoMilestones !== false) { const auto = autoTagMilestones(working); working = auto.html; milestonesAuto = auto.applied; }
  ...
  if (opts.inputPath) {
    ...
    if (looksBinary) {
      throw new Error(
        "`input_path` doesn't look like an HTML file (binary content detected: " + path.basename(opts.inputPath) + "). " +
