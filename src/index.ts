  ...
  async (params) => {
    try {
      ...
      if (params.batch) {
        ...
        const human =
          "Batch: " + report.succeeded.length + "/" + report.total + " package(s) créés dans " + outDir + "\n" +
        ...
        return { content: [{ type: "text", text: human }], structuredContent: report as unknown as Record<string, unknown> };
      }
      ...
      const human =
        "SCORM " + (result.scormVersion === "1.2" ? "1.2" : "2004 4th Edition") + " package created.\n" +
        "File: " + outputPath + "\n" +
...
// --------------------------------------------------------------------------
// batch packaging: every immediate child of a directory = one course
// --------------------------------------------------------------------------

interface BatchEntry { course: string; file_name: string; output_path: string; scorm_version: string; milestone_count: number; format: string; size_bytes: number; warnings: string[] }
interface BatchReport { total: number; succeeded: BatchEntry[]; failed: Array<{ course: string; error: string }>; report_path: string }

function titleFromEntry(name: string): string {
  return name
    .replace(/\.(zip|html?)$/i, "")
    .replace(/[-_]+/g, " ")
    .replace(/\s+/g, " ")
...
async function packBatch(params: Record<string, unknown>, outDir: string): Promise<BatchReport> {
  ...
  const succeeded: BatchEntry[] = [];
  const failed: Array<{ course: string; error: string }> = [];
  for (const name of entries) {
    try {
      const result = await buildPackage({
        inputPath: path.join(root, name),
        title: prefix + titleFromEntry(name),
        language: (params.language as string) ?? "fr-FR",
        autoMilestones: params.auto_milestones !== false,
        successOnCompletion: params.success_on_completion === true,
        masteryScore: params.mastery_score as number | undefined,
        vendorCdn: params.vendor_cdn as boolean | undefined,
        format: params.format as "auto" | "self-contained-html" | "claude-design" | undefined,
        scormVersion: params.scorm_version as "2004" | "1.2" | undefined,
      });
      const outputPath = path.join(outDir, result.fileName);
      await fs.writeFile(outputPath, result.zip);
      succeeded.push({
  ...
  const report: BatchReport = { total: entries.length, succeeded, failed, report_path: path.join(outDir, "batch-report.json") };
...
function cliArg(args: string[], name: string): string | undefined {
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (cmd === "ui") {
    ...
    return new Promise<number>(() => {}); // serve until Ctrl-C
  }
  if (cmd === "validate") {
    const target = args.find((a) => !a.startsWith("--"));
    if (!target) { console.error("validate: missing <file.zip>\n"); console.log(CLI_HELP); return 1; }
    const { validatePackage, formatReport } = await import("./validate.js");
  ...
  if (cmd !== "pack") { console.log(CLI_HELP); return cmd && cmd !== "--help" && cmd !== "-h" ? 1 : 0; }
  ...
  if (!input) { console.error("pack: missing <input>\n"); console.log(CLI_HELP); return 1; }
  const outDir = resolveOutputDir(cliArg(args, "--out"), process.env.SCORM_OUTPUT_DIR);
  await fs.mkdir(outDir, { recursive: true });
  const common = {
    ...
    scormVersion: (cliArg(args, "--scorm-version") === "1.2" ? "1.2" : "2004") as "2004" | "1.2",
  };
  if (args.includes("--batch")) {
    ...
    for (const r of report.succeeded) { console.log(" ✓ " + r.course + " → " + r.output_path); }
    for (const f of report.failed) { console.error(" ✗ " + f.course + " : " + f.error); }
  ...
  if (result.warnings.length) { for (const w of result.warnings) { console.error(" ! " + w); } }
  return 0;
}
