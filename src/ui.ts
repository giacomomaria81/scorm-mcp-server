...
export interface UiOptions { port?: number; outDir: string; version: string }
...
function readBody(req: http.IncomingMessage): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (c: Buffer) => {
      size += c.length;
      if (size > MAX_BODY) { reject(new Error("Payload too large (max 200 MB)")); req.destroy(); return; }
      chunks.push(c);
    });
...
export async function startUi(opts: UiOptions): Promise<http.Server> {
    ...
    try {
      ...
      if (req.method === "POST" && url.pathname === "/api/pack") {
        ...
        if (!body || typeof body.name !== "string" || typeof body.data !== "string") {
          json(res, 400, { ok: false, error: "Expected { name, data(base64), ... }" }); return;
        }
  ...
  await new Promise<void>((resolve) => server.listen(opts.port ?? 3117, "127.0.0.1", resolve));
  return server;
}
...
const PAGE = (version: string): string => `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<main>
  ...
  <div class="drop" id="drop" role="button" tabindex="0" aria-label="Choose a file">
    ...
    <div class="hint">.html &middot; .zip (bundle or Claude Design .dc)</div>
  </div>
